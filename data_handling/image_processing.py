from data_handling.download_from_database import DownloadTables
import pandas as pd
import torch
import numpy as np 
...
from data_handling.clean_tabular import CleanTabular
...
class PrepareData:
    ...
    def form_arrays(self, images_folder, image_size:int, n:int = None):
        ...
        for index in range(n):
            image = images[index]
            try:
                features, label = self.convert_to_image_array(image, images_folder)
                X[index, :] = features
